...
class ModerationCog(commands.Cog):
    ...
    async def mute_command(
        ...
        if user.guild_permissions.administrator:
            await response.send_message(
                content=t("command.mute.target_admin"), ephemeral=True
            )
            entry(
                *location,
        ...
        if seconds or minutes or hours or days:
            muted_until += datetime.timedelta(
        ...
        if not reason:
            content = t(
                "command.mute.success_no_reason",
                target=user.mention,
                until=muted_until_str,
            )
        else:
            content = t(
    ...
    async def unmute(self, interaction: Interaction, user: Member, reason: str = None):
        ...
        await get_response(interaction).send_message(
            t("command.unmute.success", target=user.mention), ephemeral=True
        )
        await user.edit(timed_out_until=None)
    ...
    async def poll_remind(self, interaction: Interaction, role: Role, message_id: str):
        ...
        if pings:
            response = t("command.poll_remind.success", pings=pings)
            await msg.reply(pings)
        else:
            response = t("command.poll_remind.no_pings")

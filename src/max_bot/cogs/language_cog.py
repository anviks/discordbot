...
class LanguageCog(commands.Cog):
    ...
    def set_language(
        ...
        **format_kwargs,
        ...
        return t(
            f"command.set_language.{entity_type}.success",
            language=t(f"language.{language}"),
            **format_kwargs,
        )

    @set_language_group.command(
        name="server", description="Set the language for the current server."
    )
    @app_commands.describe(language="The language to set.")
    @app_commands.guild_only
    @app_commands.checks.has_permissions(manage_guild=True)
    async def set_language_server(
        self, interaction: Interaction, language: ValidLanguages
    ):
        message = self.set_language(
            interaction, language, interaction.guild_id, "server"
        )
        await get_response(interaction).send_message(message)

    @set_language_group.command(
        name="category", description="Set the language for a category."
    )
    @app_commands.describe(language="The language to set.")
    ...
    async def set_language_category(
        ...
        message = self.set_language(
            interaction, language, category.id, "category", category=repr(category.name)
        )
        await get_response(interaction).send_message(message)

    @set_language_group.command(
        name="channel", description="Set the language for a channel."
    )
    @app_commands.describe(language="The language to set.")
    ...
    async def set_language_channel(
        self, interaction: Interaction, language: ValidLanguages, channel: TextChannel
    ):
        message = self.set_language(
            interaction, language, channel.id, "channel", channel=channel.jump_url
        )
        await get_response(interaction).send_message(message)

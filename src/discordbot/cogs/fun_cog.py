...
class FunCog(commands.Cog):
    ...
    async def ping(self, interaction: Interaction, target: Member, count: int = 1):
        ...
        await response.send_message(
            content=t(
                "command.ping.success", count, target=target.mention, count=count
            ),
            ephemeral=True,
        )
    ...
    async def say(
        self, interaction: Interaction, message: str = "​", replying_to: str = None
    ):
        t = self.translator.get_translator(interaction)
        response = get_response(interaction)
        ...
        await response.send_message(content=t("command.say.success"), ephemeral=True)

        if replying_to:
            replied_msg: Message = await interaction.channel.fetch_message(
                int(replying_to)
            )
    ...
    async def dm(
        ...
        t = self.translator.get_translator(interaction)
        await get_response(interaction).send_message(
            t("command.dm.success", target=target.mention), ephemeral=True
        )
        entry(
            *get_location(interaction),
            f'{interaction.user} ordered me to DM {target} "{message}"',
        )
